...
def bb_hw_to_corners(boxes):
    ...
    x_1s = torch.unsqueeze(boxes[:, :, 0] - x_shift[:, :, 0], dim=2)
    x_2s = torch.unsqueeze(boxes[:, :, 2] + x_shift[:, :, 1], dim=2)
    y_1s = torch.unsqueeze(boxes[:, :, 1] - y_shift[:, :, 0], dim=2)
    y_2s = torch.unsqueeze(boxes[:, :, 3] + y_shift[:, :, 1], dim=2)
    corner_boxes = torch.cat((x_1s, y_1s, x_2s, y_2s), dim=2)
    return corner_boxes
...
def iou_coefficients(predicted_boxes, target_boxes):
    predicted_boxes = bb_hw_to_corners(predicted_boxes).flatten(start_dim=0, end_dim=1)
...
def ssd_item_loss(pred_classes, pred_boxes, target_classes, target_boxes, anchor_boxes):
    corner_target_boxes = np.divide(target_boxes, image_size)
    activated_boxes = bb_activation_to_prediction(pred_boxes, anchor_boxes, grid_size)
